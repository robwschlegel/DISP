...
map3 <- ggplot(north_area, aes(x = lon, y = lat, group = grid)) +
 geom_polygon(data = north_grid, fill = "navy", colour = "black", alpha = 0.7) +
  ...
  labs(x = "Longitute", y = "Latitude") +
  theme(legend.text = element_text(size = 1),
        axis.text = element_text(size = 1))#use for figures in report paper

map <- ggarrange(map1, map2, map3, nrow = 3, ncol = 1)

ggsave(filename = "Sarah/map.png", plot = map, height = 6, width = 4)
...
ggplot(data = south_MHW_res, aes(x = mean_CPUE, y = duration)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  geom_label(aes(label = paste0("r = ",round(south_cor[1,2], 2)), x = 1.2, y = 20))
    
ggplot(data = fundy_MHW_res, aes(x = mean_CPUE, y = duration)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  geom_label(aes(label = paste0("r = ",round(fundy_cor[1,2], 2)), x = 1.2, y = 20))
